...
// Asegurarse de que estos imports estén en la parte superior del archivo
...
const initializeLocalStorage = (users, messages, educationalContent) => {
  try {
    ...
    if (!getFromStorage('messages') || getFromStorage('messages').length === 0) {
      saveToStorage('messages', messages);
    }
    if (!getFromStorage('educationalContent') || getFromStorage('educationalContent').length === 0) {
      saveToStorage('educationalContent', educationalContent);
    }
  } catch (e) {
...
const App = () => {
  ...
  useEffect(() => {
    // Pasar los mocks como argumentos a la función de inicialización
    ...
    if (storedUser) {
      ...
      // Asegurarse de que 'users' se obtenga de localStorage o del mock inicial
      ...
      } else if (storedUser.role === 'medical_viewer' || storedUser.role === 'pharmacist_viewer') {
  ...
  const handleLogin = (user) => {
    ...
    if (user.role === 'patient') {
      ...
      setCurrentView('pharmacistDashboard');
    } else if (user.role === 'medical_viewer' || user.role === 'pharmacist_viewer') {
      setCurrentView('personalPublications');
    }
  ...
  const renderView = () => {
    ...
    switch(currentView) {
      ...
      case 'personalPublications':
        return currentUser && (currentUser.role === 'medical_viewer' || currentUser.role === 'pharmacist_viewer') ? (
