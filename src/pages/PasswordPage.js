...
const PasswordPage = ({ onLogin, selectedRole, onCancel }) => {
  ...
  const handleLoginAttempt = () => {
    const users = getFromStorage('users') || initialMockUsers; // Usar el nombre renombrado
    ...
    if (selectedRole === 'pharmacist') {
      ...
      if (password === '12') {
        userToLogin = { id: 4, name: "Personal Médico", role: "medical_viewer", avatar: "PM" };
        onLogin(userToLogin);
      } else if (password === '123') {
        userToLogin = { id: 5, name: "Personal Farmacéutico", role: "pharmacist_viewer", avatar: "PF" };
        onLogin(userToLogin);
      } else if (password === '1234') {
        userToLogin = { id: 6, name: "Administrador", role: "admin", avatar: "AD" };
        onLogin(userToLogin);
      } else {
        setError('Contraseña incorrecta');
      }
